...
pub struct Coords<T> where T: Clone {
    x: T,
    y: T,
}

impl<T> Coords<T> where T: Clone {
...
impl<T> Coords<T> where T: AddAssign + SubAssign + Copy + Toi32 + Tof32 {
...
impl<T> Sub for Coords<T> where T: Sub<Output = T>, T: Copy {
...
impl<T> Add for Coords<T> where T: Add<Output = T>, T: Copy {
    type Output = Coords<T>;
...
impl<T> AddAssign for Coords<T> where T: AddAssign + Copy {
...
impl<T> SubAssign for Coords<T> where T: SubAssign + Copy {
